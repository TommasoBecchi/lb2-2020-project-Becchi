...
def predicting(prof,info_h,info_e,info_c):
	...
	for i in prof.keys():
		...
		if max(prob_h,prob_e,prob_c)==0: s+="-"
		elif max(prob_h,prob_e,prob_c)==prob_h: s+="H"
		elif max(prob_h,prob_e,prob_c)==prob_c: s+="-"
		else: s+="E"
	return s	

def principale (nome,mod,dir_output):
	prof=nome+".txt"
	'''header=prof.replace(".","")
	...
	nome_out=dir_output+nome+".pred"
	fres=open(nome_out,"w")
	contenuto=first+"\n"+result
...
if __name__=='__main__':
	...
	for nome in f:
		nome=nome.rstrip()
		principale(nome,mod,dir_output)
